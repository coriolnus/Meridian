...
## Difference from Breadth Chart Analyst (archived)
...
`breadth-chart-analyst` no longer ships as a separate skill — it was folded into this one on
2026-07-30 (see the last section). This table now reads as *why* the reproducible path won.
...
## Folded in: breadth-chart-analyst (2026-07-30 skill audit)

`breadth-chart-analyst` was archived to `skills/_emekli/breadth-chart-analyst/` (not deleted): it
duplicated this skill's job inside the same P1_REGIME chain and had zero real runs. Its core
intuitions, kept here:

**Dual timeframe.** The 200-day-MA-based S&P 500 Breadth Index is the *strategic* view; the Uptrend
Stock Ratio (stocks above the 200/50/20-day MAs with positive 1-month performance) is the *tactical*
swing view. Read them together — one alone misleads.

**Strategic levels.** 73% = average peak, i.e. overheating; 23% = average 8MA trough, i.e. extreme
oversold. Its backtested pairing: BUY when the 8MA turns up out of a trough (especially below 23%),
SELL when the 200MA rolls over into a peak (typically at or above 73%).

**Tactical levels.** ~10% = short-term bottom, ~40% = short-term top. Enter long on the downtrend→
uptrend flip (best out of <10-15%), exit on the uptrend→downtrend flip (especially from >35-40%);
horizon is days to weeks.

**Reading errors it warned about.** Confusing the 8MA with the 200MA; reading a historical trend
instead of the current direction; missing a cross that is still forming; ignoring the shaded
downtrend band; calling a reversal before the MA has actually turned.

**Engine reality (do not overstate this).** `regime.py` computes its own index-derived breadth proxy
and labels it as a proxy; ROADMAP §5 forbids making breadth a *primary* gate (the only exception is
the <20% washout case). So the thresholds above are the specification to measure against when the Y3
regime gate is engineered — they are not a live gate today, and this skill still has no engine
implementation.
