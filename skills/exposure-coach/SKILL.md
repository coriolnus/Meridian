...
| ftd-detector | `ftd_*.json` | Follow-Through Day quality (market bottom confirmation) — ARCHIVED 2026-07-30, methodology now in `macro-regime-detector`; the engine's stand-in is `regime.py follow_through()` |
| theme-detector | `theme_detector_*.json` or `theme_*.json` | Active investment themes and rotation |
| sector-analyst | `sector_*.json` | Sector performance rankings — RETIRED 2026-07-30 (§5 refuted the sector-edge path; Y3 needs sector *membership*, not this skill) |
