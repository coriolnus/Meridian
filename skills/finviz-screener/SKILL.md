...
Translate natural-language stock screening requests into FinViz screener filter codes, then either **fetch the results straight into the terminal** or open the screener in Chrome.

**Two paths, decided by the key:**

| | With `FINVIZ_API_KEY` (Elite) | Without a key |
|---|---|---|
| Default action | Fetch the CSV export and print the rows — **no browser** | Build the URL and open Chrome (unchanged) |
| Results land in | stdout (aligned table), optionally a CSV file | The browser window |
| Override | `--open` also opens Chrome; `--url-only` prints the URL only | `--fetch` errors out (exit 2) — Elite export needs a key |

The key is looked up in `$FINVIZ_API_KEY` and in the operator's local secret store.

**Key Features:**
- Natural language → filter code mapping (Japanese + English)
- Direct CSV fetch when Elite is available — results in the terminal, no Chrome round-trip
- URL construction with view type and sort order selection
- Elite/Public auto-detection (environment variable, secret store, or explicit flag)
- Chrome-first browser opening with OS-appropriate fallbacks
- Strict filter validation to prevent URL injection

**Where the export endpoint lives:** the script never builds an export URL itself. The canonical
`/export/screener` path, the auth token, redirect-following and the token masking in error messages
are all owned by `meridian/adapters/finviz.py` — one source of truth, shared with Meridian's own
autonomous discovery. (The older `.ashx` export endpoint is stale: it answers with a 301 and an
empty body, which reads as "your token doesn't work". Do not reintroduce it here.)
...
Same command either way — the script picks the path based on whether a key is available:

```bash
# With a key: fetches the CSV export and prints the rows. Without one: opens Chrome.
...
```bash
# Show 50 rows instead of the default 25
python3 scripts/open_finviz_screener.py \
  --filters "cap_small,fa_div_o3" --fetch --limit 50

# Save the complete result (all rows, all columns) and still print the head
python3 scripts/open_finviz_screener.py \
  --filters "ta_sma50_pa,ta_perf_13wup" --view technical --fetch --csv /tmp/momentum.csv

# Fetch AND open the screener in Chrome
python3 scripts/open_finviz_screener.py --filters "cap_midover" --open
```
...
- `--elite`: Force Elite mode in the URL (auto-detected from `$FINVIZ_API_KEY` if not set)
- `--view`: View type — overview, valuation, financial, technical, ownership, performance, custom. **In fetch mode the view also decides which columns the CSV carries** (overview → ticker/company/sector/price/change…, technical → SMA/RSI/ATR…). Pick the view that holds the numbers you need.
- `--order`: Sort order (e.g., `-marketcap`, `dividendyield`, `-change`). URL mode only; in fetch mode it is reported as ignored, not silently dropped.
- `--url-only`: Print the URL without fetching and without opening a browser
- `--fetch`: Force the CSV fetch. Without a key this fails loudly (exit code 2) instead of pretending
- `--open`: Open the browser as well, even when fetching
- `--limit N`: Rows to print in fetch mode (default 25). `--csv` is unaffected
- `--csv PATH`: Write the full result — every row, every column — to `PATH`

At least one of `--filters`, `--themes`, or `--subthemes` must be provided.

### Step 5: Report Results

In fetch mode, report:
1. The number of rows returned (and where the CSV was written, if `--csv` was used)
2. The notable names from the printed table, using the column headers exactly as FinViz sent them — never rename or invent a column
3. Summary of applied filters and the view used
4. Suggested next steps (e.g., "Switch to `--view financial` for payout and growth columns", "Tighten with `sh_avgvol_o1000`")

A `0 rows` result means the request succeeded and nothing matched — that is a real answer, not a
failure. An error line (`Error: …`) means the fetch itself failed; say which one happened.
...
- `scripts/open_finviz_screener.py` — URL builder, Elite CSV fetcher, and Chrome opener
- `meridian/adapters/finviz.py` — the single source of truth for the Elite export endpoint, token handling and error masking (`export_rows` is the function this skill calls)
