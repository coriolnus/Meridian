...
NOTE (AI-Trading repo, 2026-07-30): this repository does not contain the
SSoT (skills-index.yaml + workflows/ were never migrated here), so this
script — including --check — is inert by design and the committed snapshot
is frozen. Operational honesty (archived skills) is enforced at read time by
the retirement gate in recommend.py, fed by build_retirement_digest.py.
"""
