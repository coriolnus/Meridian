...
Retirement facts resolution order (operational layer, orthogonal to the
catalog metadata above; see tests/test_navigator_retirement_gate_v126.py):
  1. state/skills_registry.json under --project-root  (Claude Code, live)
  2. bundled assets/retirement_digest.json            (Web App, as-of package)
  3. none — gate disabled; the output carries an explicit verify-caveat

The gate is applied inside recommend() as a pure function of (metadata,
facts): archived skills never appear in actionable fields (setup bundles,
manifests, suggested skills), and a workflow with an archived required skill
is itself treated as archived — honest gap when primary, excluded-and-named
in `rationale` when secondary. Identical facts therefore produce identical
recommendations on both metadata paths; the two environments differ only in
the declared `retirement.source`.
"""
...
BUNDLED_RETIREMENT = SKILL_ROOT / "assets" / "retirement_digest.json"
...
# ---------------------------------------------------------------------------
# Retirement facts — operational layer over the catalog metadata.
#
# The catalog says what a skill IS; the registry says whether it is still
# live. The two are kept separate on purpose: the snapshot is a frozen
# generated artifact (never hand-edited), while retirements happen in
# state/skills_registry.json without regenerating anything.
# ---------------------------------------------------------------------------


@dataclass(frozen=True)
class RetirementFacts:
    """Which skills are archived, per the best available source.

    source: "registry" (live state/skills_registry.json), "bundled"
    (assets/retirement_digest.json, frozen at package time), or
    "unavailable" (gate disabled — output must carry a verify-caveat).
    retired maps skill id -> merged_into target (None for a plain retire).
    """
...
NO_RETIREMENT_FACTS = RetirementFacts("unavailable", None, {})


def _facts_from_registry_entries(entries: dict[str, Any], source: str) -> RetirementFacts:
    retired: dict[str, str | None] = {}
    as_of = ""
    for name, info in entries.items():
        if not isinstance(info, dict) or not info.get("retired"):
            continue
        retired[str(name)] = str(info["merged_into"]) if info.get("merged_into") else None
        as_of = max(as_of, str(info.get("retired_at") or ""))
    return RetirementFacts(source, as_of or None, retired)


def load_retirement(project_root: Path, *, digest_path: Path | None = None) -> RetirementFacts:
    """Best-effort, read-only. Registry first, bundled digest second.

    The live worker writes the registry locklessly, so a torn/partial read is
    possible — any parse failure falls through to the bundled digest rather
    than aborting the recommendation.
    """
    registry_path = project_root / "state" / "skills_registry.json"
    if registry_path.is_file():
        try:
            data = json.loads(registry_path.read_text(encoding="utf-8"))
            entries = data.get("skills")
            if isinstance(entries, dict):
                return _facts_from_registry_entries(entries, "registry")
        except (OSError, ValueError):
            pass
    digest = digest_path or BUNDLED_RETIREMENT
    if digest.is_file():
        try:
            ...
            if isinstance(raw, dict):
                retired = {
                    str(name): (
                        str(info["merged_into"])
                        if isinstance(info, dict) and info.get("merged_into")
                        else None
                    )
                    for name, info in raw.items()
                }
                as_of = data.get("as_of")
                return RetirementFacts("bundled", str(as_of) if as_of else None, retired)
        except (OSError, ValueError):
            pass
    return NO_RETIREMENT_FACTS


@dataclass(frozen=True)
class RetirementGate:
    """Result of applying RetirementFacts to a metadata dict.

    archived_workflows: workflow id -> {"category": skillset category the
    workflow belonged to, "archived_required": required skills now archived}.
    hidden: "workflow:<id>" / "skillset:<id>" -> names stripped from that
    entity's non-required tiers (surfaced in `rationale` when the entity is
    part of the recommendation, so nothing disappears silently).
    """

    facts: RetirementFacts
    archived_workflows: dict[str, dict[str, Any]]
    hidden: dict[str, list[str]]


def apply_retirement_gate(
    metadata: dict[str, Any], facts: RetirementFacts
) -> tuple[dict[str, Any], RetirementGate]:
    """Pure function: strip archived names from the catalog.

    A workflow with an archived REQUIRED skill cannot run and is removed
    wholesale (recommend() turns it into an honest gap / named exclusion).
    Archived names in optional/recommended tiers and archived workflow ids in
    prerequisite/related lists are stripped and recorded in `hidden`.
    """
    retired = facts.retired
    if not retired:
        return metadata, RetirementGate(facts, {}, {})

    skills_by_id = {s["id"]: s for s in metadata["skills"]}
    live_skills = [s for s in metadata["skills"] if s["id"] not in retired]

    archived_wfs: dict[str, dict[str, Any]] = {}
    survivors: list[dict[str, Any]] = []
    for wf in metadata["workflows"]:
        archived_required = [s for s in wf["required_skills"] if s in retired]
        if archived_required:
            first = next((s for s in wf["required_skills"] if s in skills_by_id), None)
            archived_wfs[wf["id"]] = {
                "category": skills_by_id[first]["category"] if first else "meta",
                "archived_required": archived_required,
            }
        else:
            survivors.append(wf)
    ...
    workflows: list[dict[str, Any]] = []
    for wf in survivors:
        gone = [s for s in wf["optional_skills"] if s in retired]
        gone += [p for p in wf["prerequisite_workflows"] if p in archived_wfs]
        if gone:
            hidden[f"workflow:{wf['id']}"] = gone
            wf = {
                **wf,
                "optional_skills": [s for s in wf["optional_skills"] if s not in retired],
                "prerequisite_workflows": [
                    p for p in wf["prerequisite_workflows"] if p not in archived_wfs
                ],
            }
        workflows.append(wf)

    skillsets: list[dict[str, Any]] = []
    for ss in metadata.get("skillsets") or []:
        tiers = ("required_skills", "recommended_skills", "optional_skills")
        gone = [s for tier in tiers for s in ss[tier] if s in retired]
        gone += [w for w in ss["related_workflows"] if w in archived_wfs]
        if gone:
            hidden[f"skillset:{ss['id']}"] = gone
            ss = {
                **ss,
                **{tier: [s for s in ss[tier] if s not in retired] for tier in tiers},
                "related_workflows": [
                    w for w in ss["related_workflows"] if w not in archived_wfs
                ],
            }
        skillsets.append(ss)

    gated = _finalize_metadata(live_skills, workflows, skillsets)
    return gated, RetirementGate(facts, archived_wfs, hidden)
...
def recommend(
    ...
    *,
    ...
    retirement: RetirementFacts | None = None,
) -> dict[str, Any]:
    """Pure function: (query, metadata, constraints, retirement) -> stable result dict.

    `metadata` is the RAW catalog (SSoT or snapshot); the retirement gate is
    applied here so archived-workflow detection sees the pre-gate ids.
    """
    facts = retirement if retirement is not None else NO_RETIREMENT_FACTS
    metadata, gate = apply_retirement_gate(metadata, facts)
    retirement_view = {"as_of": facts.as_of, "source": facts.source}
    ...
    if facts.source == "unavailable":
        rationale.append(
            "retirement facts unavailable (no state/skills_registry.json and "
            "no bundled retirement_digest.json) — recommended names may "
            "include archived skills; verify against skills/ before installing"
        )

    # An archived primary workflow cannot be recommended: it becomes an honest
    # gap in the category the workflow served, with live skills suggested.
    if not honest_gap and primary_id in gate.archived_workflows:
        arch = gate.archived_workflows[primary_id]
        rationale.append(
            f"primary workflow '{primary_id}' is archived (required skills in "
            f"skills/_emekli/: {', '.join(arch['archived_required'])}) — "
            f"treating as an honest gap"
        )
        gap_note = (
            f"No live workflow for this intent: '{primary_id}' was archived "
            f"to skills/_emekli/ (required skills archived: "
            f"{', '.join(arch['archived_required'])}). Suggested live skills "
            f"from the '{arch['category']}' category instead."
        )
        honest_gap = True
        gap_category = arch["category"]
        primary_id = None
        secondary_ids = []
    ...
    if honest_gap and gap_category is not None:
        ...
        hidden = gate.hidden.get(f"skillset:{gap_category}")
        if hidden and gap_skillset["manifest"] is not None:
            rationale.append(
                f"archived names hidden from skillset:{gap_category}: "
                f"{', '.join(hidden)} (see skills/_emekli/)"
            )
    ...
    # Archived secondaries are dropped BEFORE the no-api filter (they are no
    # longer in workflows_by_id, so any later check would silently pass them).
    for wid in secondary_ids:
        if wid in gate.archived_workflows:
            arch = gate.archived_workflows[wid]
            rationale.append(
                f"secondary '{wid}' excluded: archived to skills/_emekli/ "
                f"(required skills archived: {', '.join(arch['archived_required'])})"
            )
    secondary_ids = [wid for wid in secondary_ids if wid not in gate.archived_workflows]
    ...
    # Nothing disappears silently: name what the gate stripped from the
    # entities this recommendation actually uses.
    for owner in (
        [f"skillset:{skillset['id']}"]
        + [f"workflow:{wid}" for wid in [primary_id, *secondary_ids]]
    ):
        hidden = gate.hidden.get(owner)
        if hidden:
            rationale.append(
                f"archived names hidden from {owner}: {', '.join(hidden)} "
                f"(see skills/_emekli/)"
            )
...
def render_text(result: dict[str, Any]) -> str:
    ...
    lines.append(
        f"Retirement facts: {ret['source']}"
        + (f" (as of {ret['as_of']})" if ret["as_of"] else "")
    )
    if result["note"]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    facts = load_retirement(args.project_root)
    result = recommend(
        args.query,
        metadata,
        no_api=args.no_api,
    ...
    print(f"metadata source: {source}; retirement facts: {facts.source}", file=sys.stderr)
