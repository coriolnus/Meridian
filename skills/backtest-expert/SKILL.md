...
## Folded in: signal-postmortem (2026-07-30 skill audit)

`signal-postmortem` was archived to `skills/_emekli/signal-postmortem/` (not deleted): the engine's
counterfactual ledger plus `analytics.skill_attribution` already measure per-skill outcomes on a far
larger sample, deterministically. What the archived skill added — and what belongs in this
methodology document — is its **outcome taxonomy**: the labels a robustness review needs in order to
tell *kinds* of failure apart.

| Label | Definition |
|---|---|
| TRUE_POSITIVE | Predicted direction matched the realized return sign |
| FALSE_POSITIVE | Predicted direction was opposite the realized return |
| MISSED_OPPORTUNITY | Signal not taken, but would have been profitable |
| REGIME_MISMATCH | Signal failed because the market regime changed under it |

**Why the last two labels matter.** An average-R column cannot distinguish "the rule was wrong"
(FALSE_POSITIVE) from "the rule was right in the wrong regime" (REGIME_MISMATCH); the first calls for
a rule change, the second for a gate. Same for MISSED_OPPORTUNITY: it is evidence about *sizing and
gating*, not about the signal's edge.

**Two procedural rules worth keeping.** Let a signal mature (5+ sessions) before judging it; and
aggregate by skill AND by month, because a single month can carry an entire skill's statistics.

**Status — explicitly not implemented.** The counterfactual ledger has no FP/regime-mismatch field
today. This section is the specification for adding one; nothing above should be read as a field that
already exists or a number the engine currently reports.
