...
## Folded in: us-market-bubble-detector (2026-07-30 skill audit)

`us-market-bubble-detector` was archived to `skills/_emekli/us-market-bubble-detector/` (not
deleted): it was a third regime voice in P1 with no engine implementation, overlapping this skill and
`macro-regime-detector`. What is worth keeping is its **mechanical, data-before-impressions scoring
table** — a reference for the Y3 gate design, not a live signal.

**Order of operations (its central discipline).** Collect data → score mechanically → only then allow
a qualitative adjustment, capped at **+3 points**. Never rank a market by news articles, expert
comments, or the feel of a price rise; valuation alone is not a bubble verdict.

**Six quantitative indicators (2 points max each).**

| Indicator | 2 points | 1 point | 0 points |
|---|---|---|---|
| Put/Call ratio | < 0.70 | 0.70-0.85 | > 0.85 |
| Volatility suppression + highs | VIX < 12 AND index within 5% of 52w high | VIX 12-15 near highs | VIX > 15 OR >10% off highs |
| Margin debt (leverage) | YoY +20%+ AND all-time high | YoY +10-20% | ≤ +10% or negative |
| IPO overheating | count > 2× 5-yr avg AND median first-day +20%+ | count > 1.5× 5-yr avg | normal |
| Breadth anomaly | new high AND < 45% above 50DMA | 45-60% above 50DMA | > 60% above 50DMA |
| Price acceleration | 3-month return > 95th pct of 10 years | 85-95th pct | < 85th pct |

**Stage bands.** Normal 0-4 · Caution 5-7 · Elevated Risk 8-9 · Euphoria 10-12 · Critical 13-15,
each with progressively defensive exposure guidance.

**Why it is not a live gate.** Y3 answers the same question with deliberately simpler deterministic
rules (SPY 200-SMA gate, VIX > VIX3M). Several inputs above (margin debt, IPO statistics) are not in
Meridian's data path at all. Use this table when calibrating Y3 thresholds — do not present a Minsky
score the engine never computes.
