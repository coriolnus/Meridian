...
| Stage       | Script Path                                                        |
|-------------|--------------------------------------------------------------------|
<!-- 2026-07-30 skill audit: the stage skills below (edge-candidate-agent, edge-hint-extractor,
edge-concept-synthesizer, edge-strategy-designer) were folded into edge-pipeline-orchestrator and
their folders moved to skills/_emekli/<name>/ — archived, not deleted. The paths in this table are
kept as written because SCRIPT_PATHS in scripts/orchestrate_edge_pipeline.py resolves each stage from
the live folder first and falls back to the archive, so both layouts work. -->
