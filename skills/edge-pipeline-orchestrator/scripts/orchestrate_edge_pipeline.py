...
def _stage_script(relative: str) -> str:
    """Resolve a stage script, preferring the live skill folder and falling back to the archive.

    2026-07-30 skill audit: the four upstream stage skills (edge-candidate-agent, edge-hint-extractor,
    edge-concept-synthesizer, edge-strategy-designer) were folded into this orchestrator and their
    folders moved to `skills/_emekli/` (archived, NOT deleted). Their scripts are still the stage
    implementations, so resolution tries the live path first — a restored folder keeps working with no
    edit here — then the archive. The returned path may not exist; run_stage surfaces that honestly
    instead of this module guessing.
    """
    live = _SKILLS_PROJECT_ROOT / "skills" / relative
    if live.exists():
        return str(live)
    archived = _SKILLS_PROJECT_ROOT / "skills" / "_emekli" / relative
    return str(archived if archived.exists() else live)


SCRIPT_PATHS = {
    "auto_detect": _stage_script("edge-candidate-agent/scripts/auto_detect_candidates.py"),
    "hints": _stage_script("edge-hint-extractor/scripts/build_hints.py"),
    "concepts": _stage_script("edge-concept-synthesizer/scripts/synthesize_edge_concepts.py"),
    "drafts": _stage_script("edge-strategy-designer/scripts/design_strategy_drafts.py"),
    "review": _stage_script("edge-strategy-reviewer/scripts/review_strategy_drafts.py"),
    "export": _stage_script("edge-candidate-agent/scripts/export_candidate.py"),
}
