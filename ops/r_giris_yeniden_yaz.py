...
SÖZLEŞME KOMUT SATIRIDIR (CLAUDE.md §1, vaka 2026-08-30) — `main()` değil:
    ...
    python ops/r_giris_yeniden_yaz.py --db <defter.db> --olcum <sonuc_094_*.json> \\
...
NE YAZAR. Yalnız ölçüm JSON'undaki `olculebildi == true` satırlara, `seq` ile adreslenerek,
`extra_json` içine üç alan: `r_multiple_giris` (ölçülen yeni R), `r_giris_kaynak`
(qty_giris kaynağı + stop kaynağı) ve `r_giris_damga` (kart kimliği + tarih).
...
def hedefleri_sec(olcum: dict) -> list:
    """Ölçüm JSON'undan yazılacak satırlar: YALNIZ `olculebildi == true`."""
    ...
    return [s for s in satirlar if s.get("olculebildi") is True]
...
def main(argv=None) -> int:
    ...
    with open(a.olcum, "r", encoding="utf-8") as f:
        olcum = json.load(f)
    hedefler = hedefleri_sec(olcum)

    if a.kuru:
        print(f"KURU KOŞUM — yazım YOK. Hedef satır: {len(hedefler)}")
        for s in kuru_satirlar(hedefler, a.bugun):
            print(s)
        print(f"db={db_yolu} (açılmadı) yedek-dizin={a.yedek_dizin} (kullanılmadı)")
        return 0
