...
export const PREFERENCE_REGISTRY = {
  ...
  font: definePreference({
    values: fontKeys,
    // ŞABLONDA "geist" İDİ. Kayıt kendi barındırdığımız iki yüze daraldığı için
    // (lib/fonts/registry.ts, CSP `font-src 'self'`) o değer artık kümenin DIŞINDA;
    // bırakılsaydı `parsePreference` her açılışta varsayılanı reddedip sessizce
    // ilk seçeneğe düşerdi ya da tip hatası verirdi.
    defaultValue: "inter",
    persistence: "client-cookie",
    attribute: "data-font",
  }),
