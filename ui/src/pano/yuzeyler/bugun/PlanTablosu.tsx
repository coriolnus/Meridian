...
/* ============================================================================
   ...
   SATIRDA KARAR DÜĞMESİ YOK — BİLEREK (kuyruk yüzeyinin `OnayCekmecesi` doktrini):
   sembol hücresi TIKLANABİLİR ve planın TAM gövdesini bir çekmecede açar; onay/ret
   o kanıtın ALTINDA, çift adımlı durur (`PlanCekmecesi.tsx`). Onay geri alınamaz bir
   icra tetikliyor (`loop.operator_onay_ver` onay anında aynaya emir gönderiyor) ve
   geri alınamaz bir eylem, bir görev listesinin satır sonuna konmaz.

   SÜTUNLAR ARTIK FABRİKA: seçim geri-çağrısı hücreye ulaşmak zorunda olduğu için
   `SUTUNLAR` modül sabiti `sutunlar(sec)` fonksiyonuna dönüştü. Sabit tutup hücreden
   bir dış duruma yazmak, tablo örneğiyle seçim durumunu gizlice bağlardı.
   ============================================================================ */
...
import { PlanCekmecesi } from "./PlanCekmecesi";
...
/** Sütun tanımları. `sec` SATIRIN PLANINI çekmeceye verir — hücre kendi başına bir
 *  duruma yazmaz, çağıranın verdiği geri-çağrıyı çağırır. */
function sutunlar(sec: (p: PlanSatiri) => void): ColumnDef<DataTableFeatures, PlanSatiri>[] {
  return [
    {
      id: "ticker",
      ...
      cell: ({ row }) =>
        row.original.ticker === undefined ? (
          ...
          <button
            type="button"
            className="flex flex-col gap-0.5 text-left underline-offset-4 hover:underline focus-visible:underline"
            onClick={() => sec(row.original)}
            title="Planın tam gövdesini aç — kurulum, kapılar, seviyeler ve karar"
          >
        ...
        return (
          <div className="flex flex-col gap-0.5">
            ...
            {/* SÜRÜKLENME YALNIZ UÇ ÖLÇTÜYSE VAR: `drift_pct` `_enrich_stale_plans`te
                ve SADECE `entry_trigger > 0` iken yazılıyor. Yoksa satır
...
export function PlanTablosu({ b }: { b: BugunTam }) {
  ...
  // TAZELEYİCİ PAYLAŞILAN NABIZDAN: bu yüzey `/api/today`i kendisi istemiyor, `useBugun()`
  // üzerinden üst barla AYNI gövdeye bakıyor. Karar gönderildikten sonra tazelenmesi gereken
  // de o tek gövdedir — burada ikinci bir istek açmak, aynı ekranda iki farklı gerçek üretirdi.
  const { tazele } = useBugun();
  ...
  // SEÇİLİ PLAN SATIRIN KOPYASIDIR, KİMLİĞİ DEĞİL: kimlik tutup listede aramak, liste
  // tazelenip satır düştüğünde çekmeceyi sessizce boşaltırdı. Kopya, karar anındaki gövdeyi
  // ekranda tutar; tazeleme LİSTEYİ günceller ve fark çekmecenin altında YAZILI.
  const [secili, setSecili] = useState<PlanSatiri | null>(null);
  const SUTUNLAR = useMemo(() => sutunlar(setSecili), []);
