...
import Link, { alanKoku, usePathname } from "@/pano/rota";
...
export function NavMain({ items }: NavMainProps) {
  ...
  /* ETKİNLİK ALAN KÖKÜNDEN OKUNUR, ÖNEK EŞLEŞMESİNDEN DEĞİL (Meridian uyarlaması).
     Şablon `path.startsWith(sub.url)` yazıyordu; orada her sayfanın kendi yolu var.
     Burada yol `/karar` (yüzey) ya da `/karar/adaylar` (yüzey + bölüm çapası) olabilir
     ve çapasız hâl ÇOK DAHA SIK — operatör bir yüzeye çapasız girer. Önek eşleşmesi
     o hâlde HİÇBİR maddeyi etkin göstermezdi: kenar çubuğu, açık duran sayfayı
     işaretlemez, hata da vermezdi. Kök karşılaştırması ikisini de doğru okur. */
  const isItemActive = (item: NavMainItem) => {
    if (hasSubItems(item)) {
      return item.subItems.some((sub) => alanKoku(path) === alanKoku(sub.url));
    }

    return alanKoku(path) === alanKoku(item.url);
  };
  ...
  return (
    <>
      {/* ŞABLONUN "Quick Create" + "Inbox" BLOĞU DÜŞTÜ. İkisi de bir SaaS iskeletinin
          yer tutucusuydu ve Meridian'da karşılığı yok: burada "yeni bir şey oluşturmak"
          diye bir eylem yok — planları gece hattı üretir, operatör onaylar. Duran bir
          düğme bırakmak, basıldığında hiçbir şey yapmayan bir vaat olurdu. Gelen kutusu
          ise VAR ama yeri burası değil: `karar` yüzeyinin onay kuyruğu bölümü, ve sayısı
          kenar çubuğunda o maddenin rozetinde okunuyor. */}
      {items.map((group) => (
  ...
  return (
    <SidebarMenuBadge
      className={cn(
        ...
        badge === "new" &&
          "border-basari text-basari peer-hover/menu-button:text-basari peer-data-active/menu-button:text-basari",
