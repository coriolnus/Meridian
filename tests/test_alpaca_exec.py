...
def test_submit_plan_sizes_bracket_and_posts_paper(sandbox_state, monkeypatch):
    ...
    # yasa gap dalına düşer → limit fiyatlı emir, tavan = tetik + %1 (ATR yok, yalnız yüzde tavanı).
    assert b["side"] == "buy" and b["type"] == "limit" and b["limit_price"] == 20.2
...
def test_submit_plan_uses_stop_limit_when_price_is_still_below_the_trigger(sandbox_state, monkeypatch):
    ...
    b = cap["body"]
    # limit = 20 + min(0.5*0.2, %1*20) = 20 + min(0.10, 0.20) = 20.10 → ATR bacağı BAĞLADI
    assert b["type"] == "stop_limit" and b["stop_price"] == 20.0 and b["limit_price"] == 20.1
    assert res["law"]["offset_kaynak"] == "atr"
