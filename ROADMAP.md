...
| friksiyon dayanıklılığı | WP1 | `EDG-2026-040-friksiyon-dayaniklilik` | **measured (2026-08-22) — Ö1 DÜŞTÜ, bkz. aşağıdaki 🔴 ACİL kalem.** Şasi bayt-özdeş · kill#2 ilk koşumda DOĞRU şekilde durdurdu (öz-sınama tamamlandı, gevşetilmedi) · Aşama-2 kapısı donuk kurala göre kapalı (K−2, koşulan K=3) |
...
| kalem | WP | not |
|---|---|---|
| 🟠 **chop BÜTÇE-KAPALILIĞI — politika sorusu** (28d'den dönüştü) | WP3 | ÖLÇÜLDÜ: dd-cezası bağlaşımı chop'u 2025-06-12'den beri girişe tamamen kapatıyor (52/52 gün skor tam 25 < 40). SORU: bu KASITLI politika mı (chop'ta işlem istemiyoruz) yoksa dd-cezasının yan etkisi mi? Kasıtlıysa 28d kapısı chop dilimini beklemekten ÇIKARILMALI (yapısal boş dilim); değilse eşik kartı gerekir (adaylar ölçüm dizininde; dd cezası bounds'ta YOK). OPERATÖR + kart-önce |
| **yerel↔canlı replay YOĞUNLUK anomalisi** (28g-i teşhisinin yan bulgusu) | WP3 | aynı holdout penceresi yerel replay'de n=21, canlıda n=249 — 12×! Bağlam tuzağı sınıfı (yerel state kuru olabilir) ama ölçülmedi; yerel taban holdout'u None veriyor (n=5/30). Kart-önce ölçüm |
| **`propose_virgin_knob` canlı-params süzgeci** (Ö-48'in asıl tamiratı) | WP3 | %62 vakasının kaynağı: öneri katmanı bounds'tan seçip motor/params yüzeyine bakmıyor (hermes.py). Ö-48 süzgeci arama tarafını kapattı; öneri tarafı AYRI sınıf, kart-önce |
| 🔴 **ACİL: C+mb paketi +10 bps ek friksiyonda NEGATİF** (`EDG-2026-040` hükmü) | WP1 | **KARTIN DONUK KURALI BU KALEMİ AÇTI** ("aksi hâlde hüküm … ROADMAP'e ACİL kalem olarak girer"). Taban slip=5 → **+20.685** · slip=15 → **−3.067** · slip=25 → −13.722 · slip=35 → −25.681; ΔP&L CI'ları ÜÇÜNDE DE sıfırın dışında (CI-üst<0). Başabaş bandı **5-15 bps/bacak** (interpolasyon yok). Hasar FİYAT kaynaklı (n 885→862, seçilim değişmedi). BAĞLAM: gerçek friksiyon hâlâ n=4 TAHMİN (EDG-037 kill#1 ayakta) ama EDG-038 kanonik ölçütü giriş bacağında medyan +29 bps demişti — o doğruysa sistem slip25-35 bandındadır. Dokuz replay hükmündeki §E şerhi artık RAKAMLI. SIRADAKİ İŞ (kart-önce): (b) için 🆕 **KEŞİF BELGESİ HAZIR** (`docs/KESIF-FRIKSIYON-BILINCLI-SECILIM-2026-08-22.md`): |bps| TAHMİN EDİLEBİLİR (menzil% ρ=+0,90 · ATR% +0,82 — ATR plan alanı!), işaret değil; naif likidite hipotezi TERS (en pahalı dolumlar en likit isimlerde). (b) kartının hammaddesi hazır. (a) ✅ **KARTI YAZILDI 2026-08-22: `EDG-2026-042`** — friksiyon TAHMİNİNİ n=4'ten çıkarmak (E2 defteri doldukça kanonik ölçüt yeniden), (b) friksiyon-bilinçli seçilim/eşik tasarımı — İKİSİ DE YENİ KART İSTER; bu hüküm canlı davranışı DEĞİŞTİRMEZ (kâğıt mod + karar operatörde) |
...
| `23d` bar-içi stop varsayımı (stop slipajı SIFIR) | WP1 | **KANIT HAZIR (2026-08-22, kart Rol-1 kuyruğunda):** kod yolu doğrulandı (bar-içi stop TAM eff_stop'ta dolmuş sayılıyor — broker.py `_touch_exit`; kartlardaki eski satır atıfları evrilmiş), örneklem/etki sayıları + 23b mutabakatı `wp1_23d_kanit_2026-08-22/sonuc.json`ta. Ölçüm tasarımı adayı: edg040 deseninin stop-bacağı varyantı |
| `23e` gün-içi pencere · `23f` `gap_behavior:cancel` · `13` scale-out | WP1 | **H0→H1: TASARIM BELGESİ YAZILDI** (`docs/TASARIM-23E-23F-13-2026-08-22.md`). ÖNCÜL GÜNCELLENDİ: "1Day tek yol" kod düzeyinde doğru AMA gün-içi veri BİRİKİYORMUŞ — `state/bars_intraday` 20 dosya/186 MB, dakikalık, 249/251 ticker, açılış penceresi tam kapsanıyor; derinlik 20 seans (replay 2022'den koşar → 23e ancak YAKIN pencere için modellenebilir, tam-tarih için değil). Kart-adayı envanterler belgede |
...
| havuz tavanı `cpu−2` → `cpu−1` | WP3 | **KANIT HAZIR (2026-08-22, kart Rol-1'de):** 08-03 vakası = 2 işçi %99,9 CPU + pano 8,8-10,4 sn; cpu−2 o vakanın yamasıydı. BUGÜN ölçüldü: Ö-50 ayrımı riski OTOMATİK YOL İÇİN YAPISAL değiştirdi (öğrenme ayrı süreçte, CPUWeight=50; API'de hermes ipliği 0). İKİZ FORMÜL bulundu: sprint.py:672 kopyası tavan değişirse sessizce ayrışır — kart kapsamına girmeli. Kill adayları ölçüm dizininde (API p95 · PSI · atfsız 0,8-çekirdek patlamaları) |
...
**A2 · BİLDİRİM KANALI (N1) — ARTIK ÖN-ŞARTSIZ** _(§5-2; kaynak: OB-2 kapanışı + `docs/GECE-RAPORU-2026-08-13.md:86`)_
...
**C1 · FINVIZ Elite token** _(§5-8; WP4/WP11-A)_ — **ne bekleniyor:** satın alınsın mı kararı ·
...
**C2 · FMP plan/kota kararı** _(§5-3 + §5-7)_ — **ne bekleniyor:** plan yükseltme kararı ·
...
**C3 · QC login + notebook koşumu** _(§5 C2-4 + §5-11)_ — **ne bekleniyor:** `lean login` (QC "Fat
...
**C4 · NOUS_MODEL / beyin çeşitliliği** _(§5-1; **GEREKÇE GÜNCELLENDİ** — denetim F5)_ —
