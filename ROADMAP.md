...
- **[TSK-154] Wikipedia S&P 500 sayfasındaki 'Selected changes' tablosu kalktı → `constituents.as_of` PIT iskelesi sessizce kör; önbellek 11 hayalet boş satır yazıyor** — status: QUEUED · born: 2026-09-05 · owner: rol1 · size: S · trigger: —
  What: TSK-143 keşfi: A1 sp500_constituents.json `changes` 11 satır hepsi boş ({date:'',added:'',removed:''}); `_fetch_tables` `tables[1]` artık başka tablo; `as_of` 'her tarih = bugünkü liste' üretir (pitlaw 'besleyeni kapalı' der — tutarlı ama yazım uyarısız). İş: boş değişiklik satırı yazımını reddet + `obs.warn` (Yasa 4); değişiklik kaynağı için S&P basın bültenleri / alternatif (ölç); PIT sınıfı beyanı güncel.
