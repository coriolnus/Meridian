...
#### H1 — TASARIM VAR, ölçüm/uygulama bekliyor — **7 açık** (2026-08-31: +1 roster programı — koşan programın satırı yoktu; kapananlar `§8.T`/C-D-J'de)
...
| **WP12 Bot Roster** _(YENİ CEPHE 2026-08-31; program 2026-08-27 spec'iyle başladı, bugüne dek WP'si yoktu — dört fazı §7'de yaşıyordu)_ | **AÇIK** · 🆕 aktif | hermes bot filosu: gözetimsiz dar-görevli profiller — dikkat bütçesi (`@sef`) · sessiz arıza (`@bekci`) · amaç sorusu (`@karne`) · kalan roster | **Faz 1-4 REPO tarafında TAMAM** (skill+karar görevi · üç profil + birimleri + F9 kayıtları) · **canlı etkinleştirme OPERATÖRDE** (profil başına üç eylem; F9 2026-08-31: üçü de canlıda YOK) · **Faz 5+ rol seçimi ÖLÇÜM-ÖNCE** (`@kod` · `@ayna` aday; `@nobet` ELENDİ — ikinci token=sır; `@hipotez` BLOKE — merdiven duvarı kartı, EDG-2026-058 bağı) · öneri: canlıda ≥1 bot değer kanıtlamadan Faz 5 açılmaz | `deploy/hermes/profiles/` · `deploy/oracle-a1/meridian-{brifing,bekci,karne}.*` · `ops/{sef,bekci,karne}_*.py` | spec `2026-08-27-bot-roster-design.md` |
