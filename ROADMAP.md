...
- **[TSK-158] Hindsight Postgres parolası döndürülsün — 2026-09-05 17:5xZ Rol-1 komutu DATABASE_URL satırını terminale bastı (URL-gömülü parola, süzgeç hatası)** — status: OPERATOR · born: 2026-09-05 · owner: operator · size: S · trigger: —
  What: Veritabanı yalnız 127.0.0.1:5432'de dinliyor (dış erişim yok) ama değer oturum kaydında. İş: yeni rastgele parola (sunucuda üretilir, hiç basılmaz) → `ALTER ROLE hindsight PASSWORD` + /opt/hindsight/.env DATABASE_URL güncelle + hindsight-api restart + stats 200 doğrulaması; hafıza `sir-suzgeci-url-gomulu-parola` kuralı: yalnız beyaz-liste adlar basılır, BASE_URL deseni DATABASE_URL'i yakalar.
