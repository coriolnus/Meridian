...
- **[TSK-174] Alarm teslimi: bildirim kanalı YOK (Telegram/webhook ayarsız) + notify_undelivered.json sayacı 2026-08-23'ten beri güncellenmiyor (toplam 310; MECHANISM_STALE 208, MIRROR_DRIFT 51)** — status: OPERATOR · born: 2026-09-06 · owner: operator · size: S · trigger: —
  What: Kaynak: S2 sayfası #23/#48 + canlı ölçüm 2026-09-06 19:06Z (A1: TELEGRAM_BOT_TOKEN/CHAT_ID/MERIDIAN_WEBHOOK_URL yok; state/notify_undelivered.json mtime 2026-08-23 16:57Z). İki iş: (1) OPERATÖR — kanal kimliği (Telegram bot token + chat id ya da webhook) .env'e girer (kimlik bilgisi Rol-1 tarafından girilmez); (2) Rol-1 — sayacın 08-23'ten beri niye artmadığı (alarm yolu notify'a uğramıyor mu, sayaç yazımı kırıldı mı) teşhisi; alarm 'yazıldı ≠ ulaştı' kırılması sev-1 (korumasız/MIRROR_DRIFT/NAKED_POSITION alarmları teslim edilmiyor).
  Why: En ağır doğrulanmış çelişki: sistem alarm üretiyor, kimseye ulaşmıyor ve teslim-edilemedi sayacı da durmuş — iki katlı sessizlik.
  Ref: kullanim/celiski_dogrulama_2026-09-06.json; state/notify_undelivered.json (A1); meridian/notify*.py; EDG-2026-083.
