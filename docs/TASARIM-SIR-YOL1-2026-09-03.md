...
- **Shamir 1/1**: tek payda, tek dosya — operatör "anahtar dosyası" (tekil) dedi; aynı dizinde 3 payda "dağıtık güvenlik" vermez, yalnız
  ...
  yerine dar bir yönetici jetonu (`vault token create -orphan -period=720h -policy=meridian-admin`, PERİYODİK; haftalık `vault-admin-yenile.timer` yeniler — **tur-4 2026-09-14:** `-ttl`li jeton `renew` ile azami TTL'de ölürdü — **ölçüm 2026-09-14 tur-3:** `-orphan`sız yönetici jetonu kök iptaliyle birlikte gitti (çocuk iptali); kurtarma kökü için `vault.hcl` `enable_unauthenticated_access = ["generate-root"]` — Vault 2.0+ varsayılanı kapalı, CVE-2026-5807; yalnız loopback dinleyicide kabul edilen bedel) — iptal ve yeni jeton ROADMAP notuna (değer değil).
