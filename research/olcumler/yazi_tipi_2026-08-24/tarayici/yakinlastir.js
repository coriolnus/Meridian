(async function () {
  ...
  const SIRALAR = [
    ["INTER kesit",          "0O1lI Il1O0 0123456789", [10, 13, 28], [400]],
    ["INTER tam ss02cv01",   "0O1lI Il1O0 0123456789", [10, 13, 28], [400]],
    ["GEIST Mono kesit",     "0O1lI Il1O0 0123456789", [10, 13, 28], [400, 700]],
    ["Recursive Mono",       "0O1lI Il1O0 0123456789", [10, 13, 28], [400, 700]],
    ["Recursive Sans",       "0O1lI Il1O0 0123456789", [10, 13, 28], [400]],
    ["INTER kesit",          "ıİşğçöü IŞIK İĞNE ÇÖZÜM", [10, 13], [400]],
    ["Recursive Sans",       "ıİşğçöü IŞIK İĞNE ÇÖZÜM", [10, 13], [400]],
    ["GEIST Mono kesit",     "IŞIK İĞNE ÇÖZÜM ÜSTÜNDE ĞAZI", [10], [700]],
  ];
  const ZEMIN = [["g", "#fbf9f8", "#050505"], ["k", "#1E1E1E", "#D4D4D4"]];

  const aileler = [...new Set(SIRALAR.map(([a]) => a))];
  for (const a of aileler) { await document.fonts.load(`400 10px "${a}"`, "0O1lIıİşğçöü0123456789"); }
  for (const a of aileler) { await document.fonts.load(`700 10px "${a}"`, "0O1lIıİşğçöü0123456789"); }
    ...
    for (const [aile, metin, boylar, agirliklar] of SIRALAR) {
      for (const px of boylar) {
        for (const w of agirliklar) {
          ...
          // 1) GERÇEK punto boyunda bas (dpr=1: ölçülen şey CSS pikselidir)
