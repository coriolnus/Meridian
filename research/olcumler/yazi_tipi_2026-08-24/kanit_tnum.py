#!/usr/bin/env python3
# ═══════════════════════════════════════════════════════════════════════════════════════
# TNUM KANITLAYICI — 08-07 `tnum_proof.py`nin KOPYASI.
# ORİJİNAL: research/olcumler/yazi_tipi_2026-08-07/tnum_proof.py (DONMUŞ, dokunulmadı).
#
# NE DEĞİŞTİ: HİÇBİR SATIR. Bu dosya orijinalin BAYT-BAYT aynısıdır; yalnız bu başlık
# bloğu eklendi (shebang orijinalden devralındı, aşağıdaki gövde satır satır aynı).
# Kanıtlayıcı argv[1] olarak bir dizin, argv[2] olarak çıktı JSON'u alır — bu turda
# `kanit/ttf/` (kesitlerin TTF kabı, bkz. kanit_hazirla.py) ve `kanit/tnum.json` verilir.
#
# NİYE DEĞİŞMEDİ: ölçüm aracını ölçülen şeyle birlikte değiştirmek karşılaştırmayı bozar.
# 08-07 sayıları bu kodla üretildi; bu turun sayıları da aynı kodla üretilsin.
# ═══════════════════════════════════════════════════════════════════════════════════════
