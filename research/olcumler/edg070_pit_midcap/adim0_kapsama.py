"""EDG-2026-070 · ADIM-0 (adim_0_fizibilite) — KAPSAMA HARİTASI (ölçüm DEĞİL). EDG-018 kapısının AYNI ölçümü,
yeni kartla (halef; veri kaynağı değişmiş olabilir) ve BUGÜNKÜ arşivle.

Kart: research/cards/EDG-2026-070-pit-midcap-sagkalan-ust-sinir.yaml (selef EDG-2026-018; eşikler AYNEN: ≥40 isim, ≥3 yıl).
Kaynak betik: research/olcumler/wp_u_midcap/adim0_kapsama.py (2026-08-02) — mantık BİREBİR, yalnız yollar
parametrik (`--repo`, `--cikti`) ki A1'de (/opt/meridian) koşabilsin; ek eksen E: Alpaca IEX tarihsel SONDA
(kartın "yerel arşiv → Alpaca IEX tarihsel → yoksa düşer" kaynak sırası; `--alpaca-sonda N` ile, varsayılan 0 = çağrı yok).
Kartın `adim_0_fizibilite` alanı ölçümden ÖNCE koşulmasını ZORUNLU kılar. Bu betik yalnız
...
KART = REPO / "research" / "cards" / "EDG-2026-070-pit-midcap-sagkalan-ust-sinir.yaml"
...
def main():
    ...
    rapor: dict = {"kart_id": "EDG-2026-070", "selef_kart": "EDG-2026-018",
                   "adim": "ADIM-0 adim_0_fizibilite — KAPSAMA HARİTASI (ölçüm DEĞİL; EDG-018 kapısının tekrarı)",
                   ...
                   "yazim_beyani": "state/ altına YAZILMADI; tek çıktı `--cikti` (varsayılan bu klasördeki "
    ...
    # ------------------------------------------------------------------ EKSEN E (EDG-070 eki)
    # Kartın kaynak sırası: yerel bar arşivi → Alpaca IEX tarihsel (A1-içi) → yoksa düşer. Yerel arşiv
    # eksen B/D'dir; burada İKİNCİ basamak SONDALANIR: barı olmayan çıkmış isimlerden en çok N tanesi
    # için `adapters.alpaca.daily_bars` (1Day, iex, 2016-01-01→bugün). Çağrı YALNIZ `--alpaca-sonda N`
    # ile (varsayılan 0): kuru koşum ağa çıkmaz. Sonuç bir KAPSAMA sayısıdır, hüküm değil; N<len(barsız)
    # ise ÖRNEKLEMDİR ve öyle yazılır (uydurma yasağı — örneklemden tüme genelleme yapılmaz).
    sonda_n = int(ARGV.alpaca_sonda or 0)
    if sonda_n != 0:
        from meridian.adapters import alpaca as _alp
        adaylar = sorted(barsiz, key=lambda r: (r.get("uyelik_son") or ""), reverse=True)
        if sonda_n > 0:
            adaylar = adaylar[:sonda_n]          # -1 = barsız çıkmış isimlerin HEPSİ (kapı için gereken)
        semboller = [r["sembol"] for r in adaylar]
        bugun = dt.datetime.now(dt.timezone.utc).date().isoformat()
        try:
            cevap = _alp.daily_bars(semboller, start="2016-01-01", end=bugun)
        except Exception as e:  # sessiz-yutma DEĞİL: hata adıyla rapora düşer, sayı uydurulmaz (None)
            cevap = None
            sonda_hata = f"{type(e).__name__}: {e}"[:200]
        else:
            sonda_hata = None
        if cevap is None:
            rapor["E_alpaca_sondasi"] = {"sonda_n": len(semboller), "bar_donen_n": None,
                                        "bar_donen_n_neden": sonda_hata or "Alpaca veri ucu erişilemez/soğumada (daily_bars None)",
                                        "semboller": semboller}
        else:
            donen = {s: v for s, v in (cevap or {}).items() if v}
            # bar-geçmişi yıl = bar sayısı / ISLEM_GUNU_YIL (eksen B ile AYNI dönüşüm sabiti)
            yil_alpaca = {s: len(v) / ISLEM_GUNU_YIL for s, v in donen.items()}
            rapor["E_alpaca_sondasi"] = {
                "kaynak": "adapters.alpaca.daily_bars (1Day, iex, adjustment=split) 2016-01-01→bugün",
                "sonda_n": len(semboller), "barsiz_toplam_n": len(barsiz),
                "orneklem_mi": len(semboller) < len(barsiz),
                "bar_donen_n": len(donen),
                "bar_donen_semboller": sorted(donen),
                "ilk_bar_tarihleri": {s: (v[0].get("t") or v[0].get("date")) for s, v in donen.items()},
                "bar_gecmisi_yil": _ozet(list(yil_alpaca.values()), "Alpaca ile kapsanan çıkmış isimlerin bar-geçmişi (yıl; 2016 tabanı → en çok ~10,7)"),
                "yil_tabani_notu": "Alpaca IEX tarihsel 2016'dan başlar (kalıcı kayıt alpaca-a1-uzerinden); 2016 öncesi geçmiş bu kaynaktan ÖLÇÜLEMEZ (None), yıl sayısı alt sınırdır",
                "okuma": "Alpaca IEX tarihsel arşivi delist edilmiş isimleri kapsıyorsa bar döner; dönmüyorsa "
                         "ikinci basamak da boş → kartın 'yoksa düşer' kolu. Sayı örneklemdir, genelleme yok.",
            }
    else:
        rapor["E_alpaca_sondasi"] = {"sonda_n": 0, "bar_donen_n": None,
                                    "bar_donen_n_neden": "--alpaca-sonda verilmedi (kuru/yerel koşum; ağ çağrısı YOK)"}
...
if __name__ == "__main__":
    ...
    if isinstance(_e_n, int):
        ...
        r["kapi_sayilari"]["birlesik"] = {
            ...
            "isim_esigin_altinda": _birlesik_n < GATE_MIN_ISIM,
            "yil_esigin_altinda": (_birlesik_ort is not None and _birlesik_ort < GATE_MIN_YIL),
            "kapi_dusuyor_mu": (_birlesik_n < GATE_MIN_ISIM) or (_birlesik_ort is not None and _birlesik_ort < GATE_MIN_YIL),
            "orneklem_uyarisi": _e.get("orneklem_mi"),
